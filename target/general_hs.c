...
void target_general_hs(Module* module) {
  ...
  emit_line("");
  emit_line("import Data.Array.MArray");
  ...
  emit_line("");  
  emit_line("main :: IO ()");
  emit_line("main = runIdentityT (generalMain @IOUArray @IdentityT @IO @IORef)");
  emit_line("");    
  emit_line("generalMain :: forall a t m r. (MonadTrans t, MArray a Int m, GetPutInt (t m), MRef m r) => t m ()");
